...
public readonly partial struct status{
    ...
    status(in status s, in status prev){
        this = s;
        meta = log ? new Meta(meta, prev: new Ref(prev)) : new Meta(meta);
    }
    ...
    internal class Ref{ public status value; public Ref(in status s){ value = s; }}
